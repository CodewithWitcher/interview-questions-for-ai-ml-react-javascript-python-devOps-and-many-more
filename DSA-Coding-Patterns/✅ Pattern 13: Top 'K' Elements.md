...
Although this course uses <b>Heaps</b> to solve <b>Top 'K' Elements</b> problems, <b>JavaScript</b> does not have a built in method for <b>Heaps/Priority Queues</b>. It can be time consuming to implement a <b>Heap class</b> from scratch during interviews. In practice, many solutions use alternatives like quicksort, binary search strategies, bucket-style counting, greedy approaches, or hash maps depending on the question.
...
A <b>brute force solution</b> could be to sort the array and return the <b>largest K numbers</b>. The time complexity of such an algorithm will be `O(N*logN)` as we need to use a sorting algorithm like <b>Quicksort</b>. Can we do better than that?
